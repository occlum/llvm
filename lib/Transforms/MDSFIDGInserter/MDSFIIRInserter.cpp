...
static cl::opt<bool> 
    CheckStoreOnly("check-store-only", cl::init(true),
            cl::desc("only check store operation"),
            cl::Hidden);
static cl::opt<bool> 
    NoSFI("disable-SFI",
            cl::desc("do not check memory operation"),
            cl::Hidden);
...
void MDSFIIRInserterPass::handleTermInst(Instruction *I) {
  if(IndirectBrInst *IBI= dyn_cast<IndirectBrInst> (I)) { handleIndirectBrInst(IBI); }
}
  ...
  for (Function &F : M) {
    ...
    for (auto arg = F.arg_begin(); arg != F.arg_end(); arg++) {
      ...
      if (arg->getType()->isPointerTy()) {
        ...
        IRBuilder<> B(I);
        B.SetInsertPoint(I);
        Function *checkstore = Intrinsic::getDeclaration(curModule, Intrinsic::x86_checkstore,
                                         {arg->getType()});
...
static RegisterPass<MDSFIIRInserterPass> X("MDSFIIRInserter", "MDSFIIRInserter pass",
                                        false, false);
...
/* static RegisterStandardPasses
 * MDSFIIRInserterLoader_Ox(PassManagerBuilder::EP_ModuleOptimizerEarly, loadPass);
 */
static RegisterStandardPasses
    MDSFIIRInserterLoader_Ox(PassManagerBuilder::EP_OptimizerLast, loadPass);
static RegisterStandardPasses
    MDSFIIRInserterLoader_O0(PassManagerBuilder::EP_EnabledOnOptLevel0, loadPass);
