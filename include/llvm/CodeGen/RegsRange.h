...
class RangeInfo{
  ...
  /* RangeInfo() = delete; */
	...
	RangeInfo operator=(const Range &R){
		RangeInfo r(R);
		return r;
	}
  ...
  bool equal(const RangeInfo R){
	...
	static RangeInfo cutRange(RangeInfo &current, RangeInfo &guard){
      ...
      // get lower one for up
			int up = current.UpRange > guard.UpRange ? guard.UpRange: current.UpRange;
      // get higher one for low
			int low = current.LowRange < guard.LowRange ? guard.LowRange : current.LowRange;
			return RangeInfo(current.RangeClass, up, low);
		}
		// If current and guard are not in the same RangeClass, then if current is more small,
		// then return current
...
class RegsRange{
	...
	RegsRange(const TargetRegisterInfo &TRI){
		init(TRI);
	}
	RegsRange() = delete;
...
// RegsRange A equals B iif every Range in A equals B's Range
  ...
  void setAllUnknown(){
    for(auto &RegIt: Ranges){
      if(!RegIt.second.equal(RangeInfo(Undef)))
  ...
  void setChangedUnknown(RegsRange &R){
    for(auto &RegIt : Ranges){
      if(!RegIt.second.equal(R.Ranges[RegIt.first])){
        RangeInfo r(Unknown);
        //use setRegRange so if R use different register, the programe won't set two overlaped register
    ...
    for (MCSubRegIterator SubRegs(Reg, TRI, true); SubRegs.isValid(); ++SubRegs)
		...
		//if any of the super register has a range, then set it's range to unknown
    for (MCSuperRegIterator SuperRegs(Reg, TRI, false); SuperRegs.isValid(); ++SuperRegs)
    ...
    for (MCSubRegIterator SubRegs(Reg, TRI, false); SubRegs.isValid(); ++SubRegs)
			Ranges[*SubRegs] = (Undef);
    for (MCSuperRegIterator SuperRegs(Reg, TRI, false); SuperRegs.isValid(); ++SuperRegs)
			Ranges[*SuperRegs] = (Undef);
		Ranges[Reg] = Range;
	}
...
void merge(RegsRange &r) {
  for (auto &RegIt : r.Ranges) {
    ...
    if (RegIt.second.RangeClass != Undef) {
      ...
      for (MCSubRegIterator SubRegs(RegIt.first, TRI, true); SubRegs.isValid();
           ++SubRegs) {
        if (this->Ranges[*SubRegs].RangeClass != Undef) {
          this->Ranges[*SubRegs] =
              RangeInfo::meet(r.Ranges[*SubRegs], RegIt.second);
          finded = true;
          break;
        }
      }
      // if finded r go to next register
      if (finded)
        continue;
      ...
      for (MCSuperRegIterator SuperRegs(RegIt.first, TRI, false);
        ...
        if (this->Ranges[*SuperRegs].RangeClass != Undef) {
          this->Ranges[RegIt.first] =
              RangeInfo::meet(r.Ranges[*SuperRegs], RegIt.second);
          finded = true;
          break;
        }
      }
